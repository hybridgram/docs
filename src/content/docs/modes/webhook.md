...
## Go обработчик (опционально)

Пакет поддерживает Go обработчик для максимальной производительности:

```php
'webhook_port' => env('TELEGRAM_WEBHOOK_PORT', 9070),
```

Go обработчик работает параллельно с PHP и обрабатывает входящие запросы быстрее.
...
- **[Polling](/modes/polling/)** — альтернативный режим получения обновлений
